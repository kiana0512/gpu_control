...
Only the user-approved single-view workflow LoRA/fixed prompt are published.
...
KEY = os.environ['MODELVIEW_ROLLOUT_KEY']
assert KEY in ('modelview-single-view', 'modelview-single-view-inpaint')
FIXED_PROMPT_NODE = '40' if KEY == 'modelview-single-view' else '62'
EXPECTED_STEPS = 4 if KEY == 'modelview-single-view' else 2
BUNDLE = Path('/tmp/' + KEY + '-lora4500/manifest.yaml')
EVIDENCE = Path('/srv/gpu-control/jobs/deployment-' + KEY + '-lora4500-20260917')
SOURCE_JOB = '02272c28-8d14-47d4-943e-4dd26dd0b335' if KEY == 'modelview-single-view' else 'c5974e44-af5a-49cd-9fbf-ff6517f35d0c'
SOURCE = Path('/srv/gpu-control/jobs/2026/09/09') / SOURCE_JOB / 'input'
...
async def canary(database, manifest, template, node_id):
    ...
    async with httpx.AsyncClient(base_url=url, timeout=120) as client:
        try:
            ...
            assert prompt[FIXED_PROMPT_NODE]['inputs']['text'] == template[FIXED_PROMPT_NODE]['inputs']['text']
            assert prompt['15']['inputs']['steps'] == EXPECTED_STEPS
...
async def main():
    ...
    async with database.session() as db:
        ...
        old[FIXED_PROMPT_NODE]['inputs']['text'] = template[FIXED_PROMPT_NODE]['inputs']['text']
