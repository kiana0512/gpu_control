...
tag="${COMFY_IMAGE:-registry.local:5000/gpu-control/comfyui:projects-0.2.5}"
cache=()
...
bundle_sha="$(
  cd "${root}"
  find docker/comfyui workflows/production/modelview-inpaint/custom_nodes \
    -type f -print0 \
    | sort -z \
    | xargs -0 sha256sum \
    | sha256sum \
...
echo "插件锁指纹: ${lock_sha}"; echo "镜像内容指纹: ${bundle_sha}"
docker build "${cache[@]}" --file "${root}/docker/comfyui/Dockerfile" --tag "${tag}" --build-arg "PYTHON_VERSION=${PYTHON_VERSION}" --build-arg "COMFYUI_REPOSITORY=${COMFYUI_REPOSITORY}" --build-arg "COMFYUI_COMMIT=${COMFYUI_COMMIT}" --build-arg "BUILD_DATE=${build_date}" --build-arg "VCS_REF=${vcs_ref}" --build-arg "LOCK_SHA256=${lock_sha}" --build-arg "BUNDLE_SHA256=${bundle_sha}" "${root}"
mkdir -p "${root}/build"
