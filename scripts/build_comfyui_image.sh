...
tag="${COMFY_IMAGE:-registry.local:5000/gpu-control/comfyui:0.1.0}"
cache=()
...
lock_sha="$(sha256sum "${root}/docker/comfyui/requirements.lock.txt" "${root}/docker/comfyui/custom_nodes.lock.yaml" | sha256sum | awk '{print $1}')"
