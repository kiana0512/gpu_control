...
def test_control_plane_build_defaults_match_release_version() -> None:
    ...
    assert "ARG GPU_CONTROL_VERSION=1.6.36-retopo-fast-agent-v1" in asset_api
    ...
    assert "GPU_CONTROL_VERSION: ${ASSET_API_VERSION:-1.6.36-retopo-fast-agent-v1}" in compose
    assert "ASSET_API_IMAGE_TAG:-1.6.36-retopo-fast-agent-v1" in compose
    ...
    assert "ASSET_API_VERSION:-1.6.36-retopo-fast-agent-v1" in asset_api_service
