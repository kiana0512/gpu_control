...
def test_worker_image_carries_immutable_release_identity() -> None:
    ...
    assert "ARG ASSET_WORKER_VERSION=1.4.22-retopo-topology-v302" in dockerfile
...
def test_worker_release_defaults_are_aligned() -> None:
    ...
    assert "ASSET_WORKER_VERSION=1.4.22-retopo-topology-v302" in environment
    assert "ASSET_WORKER_IMAGE_TAG=1.4.22-retopo-topology-v302" in environment
    for compose in (control_compose, node_compose):
        assert (
            "ASSET_WORKER_VERSION: ${ASSET_WORKER_VERSION:-1.4.22-retopo-topology-v302}"
        ) in compose
        assert "GPU_CONTROL_REVISION: ${GPU_CONTROL_REVISION:-unknown}" in compose
        assert (
            "li3d/blender-worker:${ASSET_WORKER_IMAGE_TAG:-1.4.22-retopo-topology-v302}"
        ) in compose
