...
async def test_4090_modelview_specialization_claims_single_view_before_ordinary_work(
    tmp_path: Path,
) -> None:
    database = await make_database(tmp_path / "4090-single-view-specialization.db")
    await seed(database)
    now = datetime.now(UTC)
    async with database.session() as session:
        node = Node(
        ...
        session.add(
            Workflow(
                key=MODELVIEW_SINGLE_VIEW_WORKFLOW_KEY,
                display_name="ModelView Single View",
                description="interactive test",
            )
        )
        version = WorkflowVersion(
            workflow_key=MODELVIEW_SINGLE_VIEW_WORKFLOW_KEY,
            version="1",
            template={"9": {"class_type": "SaveImage", "inputs": {}}},
            parameter_schema={"type": "object"},
            bindings={},
            ...
            output_nodes=["9"],
            enabled=True,
            template_sha256="single-view-interactive",
        )
        session.add(version)
        await session.flush()
        session.add(
        ...
        session.add(
            Job(
                ...
                pinned=True,
                parameters={},
                request_hash="interactive-single-view",
                request_id="interactive-single-view",
                trace_id="interactive-single-view",
                job_dir=str(tmp_path / "interactive-single-view"),
            )
        )
        await session.commit()
    ...
    async with database.session() as session:
        async with session.begin():
            claimed = await claim_next_job(session, MODELVIEW_INPAINT_NODE_ID, 300)
        assert claimed is not None
        assert claimed[0].workflow_key == MODELVIEW_SINGLE_VIEW_WORKFLOW_KEY
    await database.close()
