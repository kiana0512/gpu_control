...
- In a layered soft-over-aggregate asset, first rebuild classification-only geometric adjacency across coincident export-split vertices when duplicate vertices exceed 20 percent or raw face-component counts are inflated. Use a tolerance no larger than `high diagonal * 1e-6`; never write that temporary weld into the source or result. Connectivity only describes safe separability and never determines the asset complexity class. Classify and copy whole recovered surfaces, never coordinate-clipped faces. Reduce deeply folded soft regions to an absolute silhouette-driven density instead of retaining a fixed percentage and use smooth shading without shape-changing smoothing. Exclude the soft region before sampling the aggregate. Build the aggregate as paired 8–24 point non-circular end-contour loops plus at most three intermediate loops; cylinders, capsules, ellipses, cones, AABB boxes and generic lobes are forbidden. If the contour extrusion is unreliable, conservatively reduce the aggregate source region instead of substituting a primitive. When that classification-only graph is one fused surface spanning both layers, do not force a region split; the server fast path uses its packaged adaptive whole-source reduction without a fixed ratio while retaining export seams so reduction cannot cross fused hard/soft boundaries.
