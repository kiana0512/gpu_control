...
- In a layered soft-over-aggregate asset, classify whole face-connected components before copying geometry. Preserve the complete adjacency of every selected soft component; never isolate cloth by coordinate clipping or partial-face selection. Exclude those components before sampling the aggregate envelope, keep covered sections behind the soft shell, and derive exposed ends from a sparse non-rectangular high silhouette rather than an AABB cap.
