...
For layered mixed assets such as cloth over wood, raw face components are not semantic evidence when export-split duplicate vertices exceed 20 percent. Rebuild classification-only adjacency across coincident coordinates within `high diagonal * 1e-6` without modifying source or output, then copy complete recovered surfaces. Never spatially clip faces. Keep at least 20 percent of deeply folded soft surfaces on the first reduction, preserve boundaries and use smooth shading without shape-changing smoothing. Exclude soft surfaces from aggregate samples. Aggregate delivery geometry must be a non-circular polygon contour extrusion (8–24 points per exposed end, at most three intermediate loops), never cylinders, capsules, cones, ellipses, AABB boxes or generic lobes; fall back to conservative high-derived reduction when the contour cannot be built reliably. If classification-only adjacency recovers one fused surface spanning both layers, the region boundary is not safe: do not retry normal/coordinate clipping. In the user-approved server fast path, invoke the packaged whole-source controlled-reduction fallback once at 50 percent, preserve export seams, and stop modeling.
