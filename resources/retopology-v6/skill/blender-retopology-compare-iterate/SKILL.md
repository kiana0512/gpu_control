...
- Use **semantic reconstruction** for mechanical and hard-surface multipart assets, broad planar forms, repeated construction, and assets assembled from distinct parts. “Simple” describes a legible component system, not permission to discard it: preserve component counts, centers, axes, attachment endpoints, negative spaces, and relative layers for bodies, lids, rims, handles, latches, hinges, feet, knobs, and openings. Keep broad planes sparse and bevels only dense enough for silhouette. Never turn a ring handle into a bar, a latch into a decorative strip, or a multipart case into a generic rounded shell. Joined or fused connectivity does not override this visible hard-surface classification.
...
For layered mixed assets such as cloth over wood, raw face components are not semantic evidence when export-split duplicate vertices exceed 20 percent. Rebuild classification-only adjacency across coincident coordinates within `high diagonal * 1e-6` without modifying source or output, then copy complete recovered surfaces. Never spatially clip faces. Connectivity describes only whether a region boundary is safe; it must never be used as the asset-complexity classification. Reduce deeply folded soft surfaces to an absolute silhouette-driven density rather than retaining a fixed percentage, preserve boundaries and use smooth shading without shape-changing smoothing. Exclude soft surfaces from aggregate samples. Aggregate delivery geometry must be a non-circular polygon contour extrusion (8–24 points per exposed end, at most three intermediate loops), never cylinders, capsules, cones, ellipses, AABB boxes or generic lobes; fall back to conservative high-derived reduction when the contour cannot be built reliably. If classification-only adjacency recovers one fused surface spanning both layers, the region boundary is not safe: do not retry normal/coordinate clipping. In the user-approved server fast path, invoke the packaged adaptive whole-source controlled-reduction fallback without a fixed ratio, preserve export seams, and stop modeling.
