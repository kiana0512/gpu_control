...
| Cloth or leather becomes torn strips over a solid aggregate proxy | One fused export surface was forcibly split by face normal or coordinate clipping, then open fragments were independently reduced | Treat one recovered fused surface as inseparable, without calling it a complex asset, and use the packaged adaptive whole-source controlled-reduction fallback without a fixed percentage |
