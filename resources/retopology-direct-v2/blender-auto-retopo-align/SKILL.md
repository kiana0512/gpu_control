...
- Use semantic reconstruction or per-component hybrid for normal hard-surface props. A simple hard-surface asset means its component system is legible, not that components may be discarded: preserve the count, centers, axes, attachment endpoints, negative spaces, and relative layers of bodies, lids, rims, handles, latches, hinges, feet, knobs, and openings. Keep broad planes sparse and use only enough bevel segments to hold silhouette. Never replace a ring handle with a bar, a latch with a decorative strip, or a multipart case with a generic rounded shell. Do not use blanket whole-object Decimate or remesh as a generic answer, including when a joined or fused source is still visibly mechanical. The source manifest's measured fragmentation gate is authoritative. A triangle-soup FBX may use direct reduction only from the server-created `SOURCE_HIGH_NORMALIZED_WORK` exact-weld copy when `normalized_work_source.qualified=true`; never weld, replace, or reduce `SOURCE_HIGH` itself.
...
- For layered cloth/leather over wood, rocks or debris, treat high duplicate-vertex ratios as export fragmentation. Rebuild adjacency only for classification and never treat fused connectivity as evidence that the asset is complex. Copy complete recovered surfaces, reduce soft regions to an absolute silhouette-driven density instead of retaining a fixed percentage, and set soft faces to smooth shading without changing their shape. Build aggregate regions from paired non-circular polygon end contours; never substitute cylinders, capsules, cones, ellipses, AABB boxes or generic lobes. If classification-only adjacency recovers one fused surface spanning both layers, do not tear it apart with normal or spatial face clipping: invoke `scripts/controlled_reduction_fallback.py` once without a fixed `--ratio` or `--prefer-normalized-work`, and stop modeling. The script chooses a sparse absolute face budget and increases it only when its lightweight construction-time shape budget requires it. Keeping the fresh source copy's export seams prevents reduction from crossing fused layer and hard-part boundaries. This source-derived fallback replaces repeated script edits and preserves the fused asset shape.
