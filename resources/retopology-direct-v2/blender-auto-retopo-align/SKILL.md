...
- For layered cloth/leather over wood, rocks or debris, treat high duplicate-vertex ratios as export fragmentation. Rebuild adjacency only for classification, copy complete recovered surfaces, retain at least 20 percent of deeply folded soft faces, and set soft faces to smooth shading without changing their shape. Build aggregate regions from paired non-circular polygon end contours; never substitute cylinders, capsules, cones, ellipses, AABB boxes or generic lobes. If classification-only adjacency recovers one fused surface spanning both layers, do not tear it apart with normal or spatial face clipping: invoke `scripts/controlled_reduction_fallback.py` once at ratio `0.50`, without `--prefer-normalized-work`, and stop modeling. Keeping the fresh source copy's export seams prevents reduction from crossing fused layer and hard-part boundaries. This source-derived fallback replaces repeated script edits and preserves the fused asset shape.
