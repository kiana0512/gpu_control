...
7. 低模创建后必须在同一个生成脚本中创建至少一个非空 UV 层；语义重建低模可按岛标记接缝并展开，禁止把无 UV 的模型写成成功。随后只允许运行一次确定性的拓扑可交付性检查、设置显示、保存和记录数量；不得自动渲染、评分、修正、重开、重试或生成第二版。低模必须为闭合流形；若缺少 UV，或存在边界/开边、游离边/顶点、重合几何、退化面、多面非流形边或不连续法线，必须以 `RETOPOLOGY_TOPOLOGY_INVALID` 让本次生成失败，禁止写成功报告。
