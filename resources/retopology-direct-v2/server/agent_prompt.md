...
2. `semantic_measurements` 只提供坐标和总体尺寸，不能证明形状或材质区域。必须直接检查工作 Blend 中 `SOURCE_HIGH` 的真实网格，识别主要轮廓、截面变化、开口、负空间、台阶、附件、接地结构以及连续软表面与密集重复堆叠区域。禁止根据对象名、文件名、全局 AABB、网格岛数量或单一面数阈值猜测模型身份，更禁止把任意模型简化成一个圆角盒、球、柱或其他通用基础体。生成前必须做一次且仅一次有界高模只读分析：输出一张不超过 512×512 的高模三分之四 Workbench 观察图，并结合文本几何/截面测量完成资产级与区域级分类。该图片只用于选择建形方法，不是低模对比、方向审查或交付门禁；不得评分、不得等待确认。完成后立即构建，不得重复测量或渲染。可写简短 `region_method_map` 作为诊断，但不得运行计划守卫，也不得让计划字段缺失中断生成。最终有效 Blend 和无破面结果仍是唯一交付门禁。
3. 每个指定高模只生成一个低模。当前用户授权的方法为 `controlled_direct_reduction`、`semantic_reconstruction` 或 `hybrid_per_component`；这项任务级授权覆盖训练资料中“直接减面仅作诊断”的旧限制，但不允许修改 `SOURCE_HIGH`、使用通用代理或把不同区域不加区分地整体处理。
   ...
   - 同时包含软/复杂区域与结构化或密集重复区域的模型：必须使用 `hybrid_per_component` 并先写清 `region_method_map`。复杂软区域使用受控减面；结构化构件使用语义重建；像木堆、石堆、碎料堆等以整体体块阅读、且单件缝隙不决定主要外轮廓的密集重复区域，使用少量多边形重建整体外轮廓包络，不逐个复制源网格岛。只有真实开口、负空间或轮廓由单件间隙控制时才保留相应分隔。
