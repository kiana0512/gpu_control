...
release-package-plan:
	...
	$(PYTHON) scripts/package_control_plane_release.py \
		...
		--worker-version "$${RELEASE_WORKER_VERSION:-1.4.16-retopology-degenerate-cleanup-v1}" \
...
verify-release-identity:
	...
	$(PYTHON) scripts/verify_release_identity.py \
		...
		--expected-worker-version "$${RELEASE_WORKER_VERSION:-1.4.16-retopology-degenerate-cleanup-v1}" \
