...
release-package-plan:
	...
	$(PYTHON) scripts/package_control_plane_release.py \
		...
		--worker-version "$${RELEASE_WORKER_VERSION:-1.4.14-retopology-uniform-scale-refine-v1}" \
...
verify-release-identity:
	...
	$(PYTHON) scripts/verify_release_identity.py \
		...
		--expected-worker-version "$${RELEASE_WORKER_VERSION:-1.4.14-retopology-uniform-scale-refine-v1}" \
