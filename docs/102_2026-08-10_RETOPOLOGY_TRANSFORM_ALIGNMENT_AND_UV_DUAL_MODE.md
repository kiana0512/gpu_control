...
用户任务 `f0340943-e74e-499b-ba19-5ae423ba7127` 的失败不是 Blender 读取或拓扑生成崩溃。正确方向
候选已经通过表面、中心、正确旋转和无镜像门禁，但尺寸误差 `0.1367986` 超过 `0.100`，旧实现只
评估 ICP 返回的单个统一缩放值后立即拒绝，没有继续寻找能同时满足表面与尺寸门禁的统一缩放。

修复保持原质量门禁不变：冻结正确旋转，以高模中心为唯一中心，解析计算可满足尺寸门禁的统一缩放
区间，并在该区间内做 25 点有界搜索；每个候选重新计算双向表面距离、中心和尺寸，只有表面
`<= 0.070`、中心 `<= 0.020`、尺寸 `<= 0.100`、无反射全部通过才交付。搜索不允许 XYZ 非等比
缩放、镜像、拓扑/UV 修改、重建或阈值放宽。v2 将搜索触发扩展到“表面或尺寸任一门禁失败”，因为
统一缩放本身可能同时改善两项；最终门禁仍完全相同。

真实候选验证使用任务 `5d2025ef-d80f-4c83-bc08-51137a8eb5fa` 在旧 Worker 上产生、旧版会拒绝的
原样 Blend。旧在线执行的表面误差为 `0.0605643`、尺寸误差为 `0.1723322`。最终 Worker
`1.4.15` 对保存候选的独立纯对齐复算结果如下：

| 项目 | 结果 |
|---|---|
| 搜索 | 26 个统一缩放候选，7 个同时通过全部门禁 |
| 选中缩放 | 相对乘数 `1.075703020391`；最终统一缩放 `1.0918091165` |
| 表面 / 中心 / 尺寸 | `0.0695762` / `1.10e-16` / `0.0777227`，全部通过 |
| 旋转与镜像 | 保持原正确旋转；proper rotation；无反射 |
| 低模保护 | 顶点、边、面、loop、拓扑 hash、材质槽及 UV 指纹前后一致 |

另一份真实坏候选 `3e76c062-356b-465d-b607-16aa1ed4fad8` 的表面误差约 `0.1484`；最终镜像复算仍
返回 `TRANSFORM_ONLY_ALIGNMENT_REJECTED`，证明搜索不会把明显不同的低模强行放行。在线自动重试
`5d2025ef-...` 重新生成了另一份低模，基线表面 `0.07337`、尺寸 `0.23319`；它在 v1 首次滚动时
仍被拒绝，并直接促成 v2 扩大安全搜索触发条件。该既有任务已耗尽两次执行，不会伪造成功；用户需在
最终 v2 上重新提交才会走完整新路径。

最终发布证据：

- 源码提交 `7e3db57f3c7b922381aeaf29a281490efa17d389` 与最终补丁
  `849bedfd839214fa6f87a526a672918b4e12e61e` 均已推送；
- 三台 Worker 均为 `1.4.15-retopology-uniform-scale-search-v2`，image ID
  `sha256:deb9db0b4d3a7d5edd2259d8378a22356bc8801ed91ef116b5a0c49b2d135b27`，OCI revision
  `849bedfd839214fa6f87a526a672918b4e12e61e`，RestartCount `0`；
- 三台后处理脚本 SHA-256 均为
  `13af9c8fdf3716cbf1d94a6a22c54d190665a99427c8290e1aca00ef9ed85a83`，对齐 Skill SHA-256
  仍为 `ea0588e81fa50772080bc19ff096ee29cb5b6dbc67cdb303b9d32cdbf6a99a78`；
- 完整回归基线 `525 passed, 12 skipped`；最终 v2 Ruff 与对齐/镜像合同专项 `10 passed`；
  Blender 5.1.2 镜像运行时验收、真实好候选通过与真实坏候选拒绝均通过；
- 三节点最终 `ACTIVE / ONLINE`，三个 Linux Asset Worker 均 `ONLINE / AUTHENTICATED / HEALTHY`；
  滚动时保留的真实抠图任务 `a607764b-6c7c-40ae-88fb-1a28f69dd1ae` 与
  `e83479ec-0c49-4e3f-b9d6-efa7eb50d523` 均 `SUCCEEDED / 100%`；三台 ComfyUI 未重启；
- 离线镜像：`/srv/gpu-control/images/retopology-uniform-scale-search-v2-849bedf/`
  `li3d-blender-worker-1.4.15-retopology-uniform-scale-search-v2.tar.zst`，`686482509` bytes，
  SHA-256 `3c154be8c8f9da5e520c707efbd68875667cd67fb72b4cd19f5f4142a68b56fb`。

本热修复没有改动 Direct V2 拓扑生成模型、参数或提示，没有改动 ImageClip/ModelViewCreator，未执行
压力测试，也没有重启三台 ComfyUI。
