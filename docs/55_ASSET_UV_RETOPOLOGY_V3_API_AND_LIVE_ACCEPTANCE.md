...
> 历史记录：本文记录 2026-07-28 的 V3 人工复核合同，不再代表当前生产接口。
> 2026-07-29 起请以 `58_2026-07-29_ASSET_V4_CLIENT_HANDOFF_AND_STABILITY.md` 为准：
> 新任务严格 QA 通过后自动交付，不再进入 `WAITING_REVIEW`，也不存在批准/驳回操作。
...
- 重拓扑完成执行后固定进入 `WAITING_REVIEW`，必须由提交方用户在自己的客户端看过三模型 × 四视图后批准；
  未批准的候选不会冒充最终游戏低模。
