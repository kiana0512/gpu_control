...
> **文档定位（2026-07-24）**：这是实现前的内部设计记录，不是调用合同。GPU Control 1.2.0
> 已完成首版生产实现，对外唯一权威文档为
> `38_GPU_CONTROL_MATTING_HANDOFF_V2.md`。首版只支持 `all_or_nothing`、
> `preserve_stem_png`、轮询/基础 SSE 和完整结果下载；不支持 callback、`best_effort`、
> `append_rgba_suffix`、`retry-failed`、对象存储输入或精确 `Last-Event-ID` 续传。本文件后文出现的
> 这些内容均是未来设想，不得据此调用当前 API。

## 0. 已落地实现与原设计的差异

- 接收格式已冻结为严格 manifest 1.0 + `ZIP_STORED`，manifest 必须显式列出全部帧；
- feeder 固定使用配置窗口（生产为 12），当前按既有租户公平队列和空闲槽调度，未引入单独的
  deficit round-robin 配置接口；
- 当前失败策略只有整批成功或整批失败，失败不发布结果包；
- 当前取消会收敛未开始和运行中帧，没有暂停、部分发布和“仅重试失败帧”的公共 API；
- 结果 ZIP、帧分页、三节点分布和 Web 单父行已完成，并已做三卡真实批次验证。
