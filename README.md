...
当前版本：`1.3.3`。三机调度、同步图片 API 与分布式序列帧批量抠图协议已落地；真实业务与压力测试在调度、统计和管理台中严格分层，测试任务只能使用真实客户暂时用不到的空闲 GPU。ImageClip 任务还会在领取前校验节点签名上报的 Git 提交、完整管线哈希和实时 ComfyUI 节点类型清单，防止旧管线、缺失插件或中间结果进入生产返回；批量父任务与子帧进度保持单调，不会因 ComfyUI 切换内部节点而倒退。
