...
`1.6.27-retopo-generated-cleanup-v1`、三台 Linux Blender Worker
`1.4.29-retopo-generated-cleanup-v1`、数据库 `20260810_0013`。当前总状态为
...
v3 同任务生成路径不运行旧自动视觉 QA，但坐标恢复、拓扑/UV 指纹、FBX 回读、身份、manifest、
文件完整性和 SHA 都是硬门禁；
...
清理模型缓存。自动拓扑对齐包 v3.0.8 统一 FBX/GLB/GLTF/OBJ 的只读高模准备入口，并由服务器确定性执行
任务中唯一且受限、但被 Codex 遗漏执行的生成脚本一次；新生成低模在 Join/曲线转换后、UV 前清理完全
重合点和退化构建残留，构建期坏候选最多重新生成一次；随后继续执行相同的 UV、拓扑、坐标和 FBX 门禁。
