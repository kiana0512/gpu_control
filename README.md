...
`1.6.40-retopo-repeated-components-v1`、三台 Linux Blender Worker
`1.4.40-retopo-repeated-components-v1`、数据库 `20260810_0013`。当前总状态为
...
清理模型缓存。自动拓扑对齐包 v3.0.18 统一 FBX/GLB/GLTF/OBJ 的只读高模准备入口，正式加载训练
拓扑技能按真实高模轮廓、组件、开口和负空间建形，并由独立对齐技能恢复坐标；仍不增加方向审查、
