...
`1.6.31-retopo-resilient-leases-v1`、三台 Linux Blender Worker
`1.4.31-retopo-resilient-leases-v1`、数据库 `20260810_0013`。当前总状态为
...
v3 同任务生成路径运行独立 FBX 形状接近度门禁并生成七方向证据；坐标恢复、拓扑/UV 指纹、FBX
回读、身份、manifest、文件完整性和 SHA 都是硬门禁；
...
清理模型缓存。自动拓扑对齐包 v3.0.9 统一 FBX/GLB/GLTF/OBJ 的只读高模准备入口，并由服务器确定性执行
